<template>
    <ion-page>
        ...
        <ion-content :fullscreen="true">
            <ion-list :inset="true">
                <!-- List -->
                <ion-item v-for="sales in sales_summary_report" :key="sales.id" >
                    <ion-grid>
                        <ion-row>
                            <ion-col size="6">
                                <ion-label>
                            ...
                            <ion-col size="6">
                                <ion-label>
                                    <p>Total Amount: {{ sales.total_amount }}</p>
                                    <p>Balance Amount: {{ sales.balance_amount }}</p>
                                </ion-label>
                            </ion-col>
                        </ion-row>
                    </ion-grid>
...
import { generateSalesSummaryReport, getSalesSummaryReport } from '@/services/report/sales-summary-report.service';
import { onIonViewDidEnter } from '@ionic/vue';
import { defineComponent, onMounted, ref } from 'vue';

export default defineComponent({
    setup(){
        const sales_summary_report = ref<SALES_SUMMARY_REPORT_DTO[]>([])
        ...
        async function handlePrint() {
            try {
                await generateSalesSummaryReport(start_date.value, end_date.value);
            } catch (error) {
        ...
        async function fetchList() {
            try {
                const result = await getSalesSummaryReport(start_date.value, end_date.value);
                if(result.success){
                    console.log(result.data)
                    sales_summary_report.value = result.data;
                }
            } catch (error) {
                await presentToast(`Operation failed${error}`)
            }
