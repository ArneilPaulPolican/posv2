...
import { STOCK_OUT_DTO } from "@/models/stock-out.model";
import jsPDF from "jspdf";

export async function generateStockOutReceipt(stock_out:STOCK_OUT_DTO, stock_out_items:STOCK_OUT_ITEMS_DTO[]) {
    const doc = new jsPDF();
    ...
    marginY += 5;
    doc.text(`IN-${stock_out.out_number}`, availableWidth / 2, marginY, null, null, 'center');

    marginY += 5;
    doc.text(`${stock_out.out_date}`, availableWidth / 2, marginY, null, null, 'center');

    marginY += 5;
    doc.text(`${stock_out.remarks}`, availableWidth / 2, marginY, null, null, 'center');
    ...
    for (let i = 0; i < stock_out_items.length; i++) {
        const item = stock_out_items[i] //items[i];
